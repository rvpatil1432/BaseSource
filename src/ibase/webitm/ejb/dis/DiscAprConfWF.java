...
public class DiscAprConfWF 
{
	public String confirm(String tranId, String empCodeAprv, String xmlDataAll, String processId, String keyFlag, String userInfoStr) throws RemoteException, ITMException
	{
	    ...
	    try {
	    	...
	    	System.out.println("Inside DiscAprWF confirm()");
	  
	    	System.out.println("Inside DiscAprWF confirm section: "+userInfoStr);
			UserInfoBean userInfo = new UserInfoBean(userInfoStr);
	    	String transDB = userInfo.getTransDB();
	    	System.out.println("get TransDB connetion in DiscAprWF : "+transDB);
	    	...
	    	{
	    		sql = "update DISC_APR_STRG set confirmed = 'Y', conf_date = sysdate where tran_id = ?";
	...
	public String escalate (String tranId, String empCodeAprv, String xmlDataAll, String processId, String roleCode, String activityId,String userInfoStr) throws ITMException, Exception {
		...
		try
		{
			...
			System.out.println("Inside DiscAprConfWF confirm section: "+userInfoStr);
			UserInfoBean userInfo = new UserInfoBean(userInfoStr);
			String transDB = userInfo.getTransDB();
			if (transDB != null && transDB.trim().length() > 0)
		    {
			...
			sql = "select SIGN_STATUS from OBJ_SIGN_TRANS "
				+ "WHERE ltrim(rtrim(REF_SER)) = 'PYALOC' AND ltrim(rtrim(REF_ID)) = ? AND ltrim(rtrim(ROLE_CODE__SIGN)) = ? "
				+ "AND SIGN_STATUS = 'U'";
			...
			if("U".equalsIgnoreCase(signStatus)){
				...
				sql = "UPDATE WF_PRC_STATUS "
						...
						+ "WHERE ltrim(rtrim(REF_SER)) = 'PYALOC' AND REF_ID = ? AND PROCESS_ID = ? AND INSTANCE_ID = ? AND ACTIVITY_ID = ? AND PROCESS_STATUS != 3";
				...
				pstmt.close();
				pstmt = null;
				System.out.println("Update DiscAprConfWF Successfully:"+resultCnt);
			} 
			else 
			{
				retString = "N";
			...
			System.out.println("Exception: DiscAprConfWF escalate method ..> " + e.getMessage());
			isError = true;
			throw new ITMException(e);
		} finally {
			if(rs !=null)
			...
			if (conn != null) {
				try {
					if (isError || !(resultCnt > 0)) {
						conn.rollback();
						retString = "N";
						System.out.println("DiscAprConfWF escalate: Connection Rollback");
					} else {
						conn.commit();
						retString = "Y";
						System.out.println("DiscAprConfWF escalate: Connection Commit");
					}
					conn.close();
					conn = null;
				} catch (Exception e) {
