...
/*
 * 
 * In this code,New purchase amendment transaction going to create for change status of existing purchase order
 * we are passing XML for purchase amendment transaction
 *   */
...
@Stateless
public class PorderClosePrc extends ProcessEJB implements PorderClosePrcLocal,
		PorderClosePrcRemote {
	...
	//DistCommon distCom=new DistCommon();
	...
	// getData Method
	public String getData(String xmlString, String xmlString2,
			String windowName, String xtraParams) throws RemoteException,
			ITMException {
		System.out.println("PorderClosePrc :getData() function called");
		String rtrStr = "";
		...
		try {
			...
			System.out
					.println("Exception :PorderClosePrcEJB :getData(String xmlString, String xmlString2, String windowName, String xtraParams):"
							+ e.getMessage() + ":");
			rtrStr = e.getMessage();
	...
	public String getdata(Document headerDom, Document detailDom,
			String windowName, String xtraParams) throws RemoteException,
			ITMException {
		...
		StringBuffer PorderClosePrc = new StringBuffer();
		int TotStatusCnt = 0, totLineNo = 0;
		String purcOrder = "", headerstatus = "", deptCode_po = "", empCode = "", deptCode_userlogin = "";
		String confirmed = "", loginEmpCode = "";
		...
		try {
			...
			loginEmpCode = genericUtility.getValueFromXTRA_PARAMS(xtraParams,
					"loginEmpCode");
			System.out.println("loginEmpCode is ::" + loginEmpCode);

			if ((purcOrder == null || purcOrder.trim().length() == 0)) {
				// Empty purchase order!!!
				errString = itmDBAccessEJB.getErrorString("", "VPPURCEMP", "",
						"", conn);
				return errString;
			} else {
				...
				if (rs.next()) {
					int cnt = rs.getInt(1);
					if (cnt == 0) {
						// Purchase Order not exist.Please enter valid Purchase
						// Order
			...
			if (confirmed.equals("N") || confirmed.equalsIgnoreCase("N")) {
				// Record not Confirm
				errString = itmDBAccessEJB.getErrorString("", "VPTPORNTCF", "",
						"", conn);
				return errString;
			} else if (headerstatus.equals("X")
					|| headerstatus.equalsIgnoreCase("X")) {

				errString = itmDBAccessEJB.getErrorString("", "VPCANCC", "", "",
						conn);
				return errString;
			} else if (headerstatus.equals("C")
					|| headerstatus.equalsIgnoreCase("C")) {
				errString = itmDBAccessEJB.getErrorString("", "VPOCLOSE", "",
						"", conn);
				return errString;
			}
			...
			sql = " select count(status) from porddet where purc_order = ? and status='C'";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, purcOrder);
			rs = pstmt.executeQuery();
			if (rs.next()) {
			...
			if ((totLineNo == TotStatusCnt)) {
				...
				errString = itmDBAccessEJB.getErrorString("", "VPDNFND", "",
						"", conn);
				return errString;
			}

			/*
			 * After enter Purchase Order No, system should validate dept code
			 * of PO Emp Code & logged user dept should be same. If not, show a
			 * message as
			 * "You not authorized to view PO Details as your & Purchaser's Dept code is different."
			 */
			...
			if (rs.next()) {
				...
				System.out.println("deptCode_po-->@@" + deptCode_po);
			...
			if (rs.next()) {
				...
				System.out.println("deptCode-->@@" + deptCode_userlogin);
			}
			if (!(deptCode_po).equals(deptCode_userlogin)) {

				errString = itmDBAccessEJB.getErrorString("", "VPNOTEVAL", "",
						"", conn);
				return errString;
			...
			/*sql = "SELECT PORDDET.purc_order,PORDDET.line_no,PORDER.ord_date,porder.pord_type,porder.supp_code," +
					"supplier.supp_name,porddet.dlv_qty,porddet.rate,PORDDET.quantity,PORDDET.tot_amt,porddet.proj_code,PORDDET.site_code," +
					"PORDDET.ind_no,PORDDET.item_code,item.descr,PORDDET.status " +
					"FROM PORDDET,PORDER,item,supplier" +
					" WHERE porder.purc_order=porddet.purc_order and porddet.item_code=item.item_code and porder.supp_code=supplier.supp_code " +
					" AND porddet.STATUS='O' and porddet.purc_order=? order by porddet.line_no";
			*/
			sql="SELECT PORDDET.purc_order,PORDER.ord_date,porder.pord_type,PORDDET.line_no," +
					"PORDDET.status,PORDDET.quantity,item.descr,porddet.dlv_qty,porddet.rate," +
					"PORDDET.tot_amt,porddet.proj_code,PORDDET.site_code,PORDDET.ind_no," +
					"PORDDET.item_code,porder.supp_code,supplier.supp_name" +
					" FROM PORDDET,PORDER,item,supplier " +
					"WHERE porder.purc_order=porddet.purc_order " +
					"and porddet.item_code=item.item_code and " +
			...
			while (rs.next()) {
				...
				// tranid_purc_order
				PorderClosePrc.append(
						rs.getString(1) == null ? "" : rs.getString(1)).append(
						"\t");
				PorderClosePrc.append(rs.getDate(2)).append("\t");
				...
				PorderClosePrc.append(rs.getDouble(6)).append("\t");
				...
				PorderClosePrc.append(rs.getDouble(8)).append("\t");

				PorderClosePrc.append(rs.getDouble(9)).append("\t");

				PorderClosePrc.append(rs.getDouble(10)).append("\t");
			...
			resultString = PorderClosePrc.toString();
			...
			System.out.println("SQLException :PorderClosePrcEJB:getdata(Document headerDom, Document detailDom, String windowName, String xtraParams):"
							+ e.getMessage() + ":");
			throw new ITMException(e);
		} catch (Exception e) {
			System.out
					.println("Exception :PorderClosePrcEJB :getdata(Document headerDom, Document detailDom, String windowName, String xtraParams):"
							+ e.getMessage() + ":");
			throw new ITMException(e);
		} finally {
			try {
	...
	public String process(String xmlString, String xmlString2,
		...
		try {
			...
			System.out
					.println("Exception :PorderClosePrcEJB :process(String xmlString, String xmlString2, String windowName, String xtraParams):"
							+ e.getMessage() + ":");
			e.printStackTrace();
			retStr = e.getMessage();
			throw new ITMException(e);
	...
	public String process(Document headerDom, Document detailDom,
		...
		ITMDBAccessEJB itmDBAccessEJB = new ITMDBAccessEJB();
		String fname = "", mname = "", lname = "", crTerm = "";
		String commPercOn = "", empCode = "";
		...
		double conv_qty_stduom = 0, conv_rtuom_stduom = 0, exchRate = 0, rate_stduom = 0, rate = 0,advAmt=0;
		String specialInstr = "",vouchStatus="";
		String indNo = "", unit = "", locCode = "";
		String packInstr = "", acct_dr = "", cctr_dr = "", acct_cr = "", cctr_cr = "";
		String suppCode = "", siteCode = "";
		...
		Timestamp dlvDate = null, reqDate = null;
		String WorkOrder = "", packCode = "", discountType = "", specificInstr = "", licenceNo = "", formNo = "", dutyPaid = "";
		String unitRate = "", retString = "", unitStd = "";
		String frtType = "";
		double frtRate = 0, frtAmtQty = 0, frtAmtFixed = 0,advQty=0,sumPoQty=0.0;
		String lineNo = "",porcpConfirmed="",tranID="";
		String refDateStr = "";
		String taxDateStr = "",errCode="";
		String ordDateStr = "", lineNo1 = "", empCodeQcApr = "",amdConfirmed="",amdNo="",poRetConfirmed="";
		String purcOrder = "";
		int forcount = 0, lineCnt = 0;
		String refMulIndSt="";
		double oQty=0;
		double convIndOrdQty=0;
		...
		try {
			...
			sql = "select count(line_no) from porddet where purc_order=? AND status='O'";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, purcOrder);
			rs = pstmt.executeQuery();
			if (rs.next()) {
			...
			if (lineCnt == parentNodeListLength) {
				
				/* * Error Msg=Use Po close transaction for fully closed po.
				 * Description:LineCnt:--taking total line count of those
				 * purchase order having status is 'O' parenNodeListLenght:--
				 */
				errString = itmDBAccessEJB.getErrorString("", "VPCLOSECNF", "",
						"", conn);
				return errString;
			...
			sql = "select (case when confirmed is null then 'N' else confirmed end)," +
					"(case when adv_amt is null then 0 else adv_amt end)" +
					
					"		from voucher" +
					"		where vouch_type = 'A' and" +
					"					purc_order = ?" ;
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, purcOrder);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				vouchStatus = rs.getString(1);
				advAmt=rs.getDouble(2);
			}
			rs.close();
			rs = null;
			pstmt.close();
			pstmt = null;
			if((vouchStatus.equals("N")||vouchStatus.equalsIgnoreCase("N"))||( vouchStatus.equals("N")|| (advAmt>0) ))
			{
				errString = itmDBAccessEJB.getErrorString("", "VTPOADVUC", "",
						"", conn);
				return errString;
				
			}else if(vouchStatus.equals("Y")||vouchStatus.equalsIgnoreCase("Y"))
			{
				errString = itmDBAccessEJB.getErrorString("", "VTPOADVC", "",
						"", conn);
				return errString;
			...
			//sql = "select amd_no,confirmed from poamd_hdr where purc_order=?";
			
			sql = "select h.amd_no,h.confirmed " +
					"from poamd_hdr h,poamd_det d " +
					"where h.purc_order=d.purc_order  and h.confirmed='N' " +
					"and d.line_no__ord=? " +
					"and  h.purc_order=?";
			pstmt = conn.prepareStatement(sql);
			...
			rs = pstmt.executeQuery();
			if(rs.next()) {
				amdNo = rs.getString(1);
				amdConfirmed=rs.getString(2);
				
				if(amdConfirmed.equals("N")||amdConfirmed.equalsIgnoreCase("N"))
				{
					
					System.out.println("In Amendment validation>>");
					errCode = "VTAMDUC";
					...
					String value = " ;    Purchase order is :      " + purcOrder  
							+" ;   Amendment No :    "+ amdNo +" ; " ;
		...
		//	sql = "select tran_id,confirmed from porcp where purc_order=?";
			sql = "select h.tran_id,h.confirmed " +
					"from porcp h,porcpdet d " +
					"where h.purc_order=d.purc_order and h.confirmed='N' and h.tran_ser='P-RCP' " +
					"and h.purc_order=? " +
					"and d.line_no__ord=? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, purcOrder);
			...
			if(rs.next()) {
				...
				porcpConfirmed=rs.getString(2);
				//porcpConfirmed
				if(porcpConfirmed.equals("N")||porcpConfirmed.equalsIgnoreCase("N"))
				{
					
					System.out.println("In Reciept validation>>");
					errCode = "VTRCPUC";
					...
					String value = " ;    Purchase order is :      " + purcOrder  
							+" ;   Reciept tran_ID :    "+ tranID +" ;  Line No: "+lineNo+" ;" ;
				...
				System.out.println("rec@@tranid@@"+tranID+"lineno@@"+lineNo);
			}
				...
				sql = "select h.tran_id,h.confirmed " +
						"from porcp h,porcpdet d " +
						"where h.purc_order=d.purc_order and h.confirmed='N' " +
						"and h.purc_order=? and h.tran_ser='P-RET' " +
						" and d.line_no__ord=? ";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, purcOrder);
				pstmt.setString(2, lineNo);
				...
				if(rs.next()) {
					tranID = rs.getString(1);
					poRetConfirmed=rs.getString(2);
					
					if(poRetConfirmed.equals("N")||poRetConfirmed.equalsIgnoreCase("N"))
					{
						
						System.out.println("In purchase return validation>>");
						errCode = "VTPRETUC";
						...
						String value = " ;    Purchase order is :      " + purcOrder  
								+" ;   purchase Return tran_ID :    "+ tranID +" ; Line No: "+lineNo+" ; " ;
			...
			xmlBuff.append("<description>").append("Datawindow Root")
					.append("</description>");
			xmlBuff.append("<group0>");
			xmlBuff.append("<description>").append("Group0 description")
					.append("</description>");
			xmlBuff.append("<Header0>");
			xmlBuff.append("<objName><![CDATA[").append("porderamd")
					.append("]]></objName>");
			xmlBuff.append("<pageContext><![CDATA[").append("1")
					.append("]]></pageContext>");
			xmlBuff.append("<objContext><![CDATA[").append("1")
					.append("]]></objContext>");
			xmlBuff.append("<editFlag><![CDATA[").append("A")
					.append("]]></editFlag>");
			xmlBuff.append("<focusedColumn><![CDATA[").append("")
					.append("]]></focusedColumn>");
			xmlBuff.append("<action><![CDATA[").append("SAVE")
					.append("]]></action>");
			xmlBuff.append("<elementName><![CDATA[").append("")
					.append("]]></elementName>");
			xmlBuff.append("<keyValue><![CDATA[").append("1")
					.append("]]></keyValue>");
			xmlBuff.append("<taxKeyValue><![CDATA[").append("")
					.append("]]></taxKeyValue>");
			xmlBuff.append("<saveLevel><![CDATA[").append("1")
					.append("]]></saveLevel>");
			xmlBuff.append("<forcedSave><![CDATA[").append(true)
					.append("]]></forcedSave>");
			xmlBuff.append("<taxInFocus><![CDATA[").append(false)
					.append("]]></taxInFocus>");
			xmlBuff.append("<description>").append("Header0 members")
					.append("</description>");
			...
			xmlBuff.append("<purc_order>")
					.append("<![CDATA[" + purcOrder + "]]>")
					.append("</purc_order>");

			System.out.println("@@@@@ordDateStr" + ordDateStr);
			if (ordDateStr != null && ordDateStr.trim().length() > 0) {
				xmlBuff.append("<ord_date>")
						.append("<![CDATA[" + ordDateStr + "]]>")
						.append("</ord_date>");
			} else {
				xmlBuff.append("<ord_date>").append("<![CDATA[]]>")
						.append("</ord_date>");
			}
			...
			xmlBuff.append("<dept_code__o>")
					.append("<![CDATA[" + deptCode + "]]>")
					.append("</dept_code__o>");
			xmlBuff.append("<emp_code>").append("<![CDATA[" + empCode + "]]>")
					.append("</emp_code>");
			...
			xmlBuff.append("<emp_fname>").append("<![CDATA[" + fname + "]]>")
					.append("</emp_fname>");
			xmlBuff.append("<emp_mname>").append("<![CDATA[" + mname + "]]>")
					.append("</emp_mname>");
			xmlBuff.append("<emp_lname>").append("<![CDATA[" + lname + "]]>")
					.append("</emp_lname>");

			xmlBuff.append("<order_db>").append("<![CDATA[" + orderDb + "]]>")
					.append("</order_db>");
			xmlBuff.append("<item_ser>").append("<![CDATA[" + itemSer + "]]>")
					.append("</item_ser>");
			xmlBuff.append("<item_ser__o>")
					.append("<![CDATA[" + itemSer + "]]>")
					.append("</item_ser__o>");
			xmlBuff.append("<tax_opt>").append("<![CDATA[" + taxOpt + "]]>")
					.append("</tax_opt>");
			xmlBuff.append("<tax_opt__o>").append("<![CDATA[" + taxOpt + "]]>")
					.append("</tax_opt__o>");
			xmlBuff.append("<cr_term>").append("<![CDATA[" + crTerm + "]]>")
					.append("</cr_term>");
			xmlBuff.append("<cr_term__o>").append("<![CDATA[" + crTerm + "]]>")
					.append("</cr_term__o>");
			xmlBuff.append("<ord_amt__o>").append("<![CDATA[" + ordAmt + "]]>")
					.append("</ord_amt__o>");
			xmlBuff.append("<tax_amt__o>").append("<![CDATA[" + taxAmt + "]]>")
					.append("</tax_amt__o>");
			xmlBuff.append("<tot_amt__o>").append("<![CDATA[" + totAmt + "]]>")
					.append("</tot_amt__o>");
			xmlBuff.append("<ord_amt>").append("<![CDATA[" + ordAmt + "]]>")
					.append("</ord_amt>");
			xmlBuff.append("<tax_amt>").append("<![CDATA[" + taxAmt + "]]>")
					.append("</tax_amt>");
			xmlBuff.append("<tot_amt>").append("<![CDATA[" + totAmt + "]]>")
					.append("</tot_amt>");
			...
			xmlBuff.append("<exch_rate__o>")
					.append("<![CDATA[" + exchRate + "]]>")
					.append("</exch_rate__o>");
			xmlBuff.append("<tax_chap>").append("<![CDATA[" + taxChap + "]]>")
					.append("</tax_chap>");
			...
			xmlBuff.append("<tax_class__o>")
					.append("<![CDATA[" + taxClass + "]]>")
					.append("</tax_class__o>");
			xmlBuff.append("<tax_env>").append("<![CDATA[" + taxEnv + "]]>")
					.append("</tax_env>");
			xmlBuff.append("<tax_env__o>").append("<![CDATA[" + taxEnv + "]]>")
					.append("</tax_env__o>");
			xmlBuff.append("<remarks>").append("<![CDATA[" + remarks + "]]>")
					.append("</remarks>");
			...
			if (taxDate != null) {
				...
				xmlBuff.append("<tax_date__o>").append("<![CDATA[]]>")
						.append("</tax_date__o>");
			}
			...
			xmlBuff.append("<quot_no>").append("<![CDATA[" + quotNo + "]]>")
					.append("</quot_no>");
			xmlBuff.append("<quot_no__o>").append("<![CDATA[" + quotNo + "]]>")
					.append("</quot_no__o>");
			...
			xmlBuff.append("<tran_code__o>")
					.append("<![CDATA[" + tranCode + "]]>")
					.append("</tran_code__o>");
			xmlBuff.append("<frt_amt__o>").append("<![CDATA[" + frtAmt + "]]>")
					.append("</frt_amt__o>");
			...
			xmlBuff.append("<frt_term>").append("<![CDATA[" + frtTerm + "]]>")
					.append("</frt_term>");
			xmlBuff.append("<frt_term__o>")
					.append("<![CDATA[" + frtTerm + "]]>")
					.append("</frt_term__o>");
			xmlBuff.append("<dlv_term>").append("<![CDATA[" + dlvTerm + "]]>")
					.append("</dlv_term>");
			...
			xmlBuff.append("<supplier_stan_code>")
			...
			for (selectedRow = 0; selectedRow < parentNodeListLength; selectedRow++) {
				...
				for (int childRow = 0; childRow < childNodeListLength; childRow++) {
					...
					if (childNodeName.equals("purc_order")) {
						System.out.println("purc_order@@@ before.........:-"
								+ purcOrder);
						purcOrder = childNode.getFirstChild().getNodeValue();
						System.out.println("purc_order@@@ after.........:-"
								+ purcOrder);

					}
					if (childNodeName.equals("line_no")) {
						System.out.println("lineNo@@@ before.........:-"
								+ lineNo);
						lineNo = childNode.getFirstChild().getNodeValue();
						System.out.println("lineNo@@@ after.........:-"
								+ lineNo);
				...
				while(rs.next()) {
					count++;
					System.out.println("count@@inside detail2" + count);
					...
					// statusDate = rs.getTimestamp("status_date");
					refMulIndSt=status;
					System.out.println("refMulIndSt@@@"+refMulIndSt);
					taxClass = checkNull(rs.getString("tax_class"));
					taxChap = checkNull(rs.getString("tax_chap"));
					taxEnv = checkNull(rs.getString("tax_env"));
					remarks = checkNull(rs.getString("remarks"));
					...
					// status = checkNull(rs.getString("status"));
					// supp_code_mnfr =
					// checkNull(rs.getString("supp_code__mnfr"));
					...
					xmlBuff.append("<purc_order><![CDATA[" + purcOrder
							+ "]]></purc_order>");
					xmlBuff.append("<line_no><![CDATA[" + lineNo
							+ "]]></line_no>");
					xmlBuff.append("<line_no__ord><![CDATA[" + lineNo1
							+ "]]></line_no__ord>");
					xmlBuff.append("<site_code><![CDATA[" + siteCode
							+ "]]></site_code>");
					xmlBuff.append("<ind_no><![CDATA[" + indNo + "]]></ind_no>");
					xmlBuff.append("<item_code><![CDATA[" + itemCode
							+ "]]></item_code>");
					xmlBuff.append("<quantity><![CDATA[" + quantity
							+ "]]></quantity>");
					xmlBuff.append("<unit><![CDATA[" + unit + "]]></unit>");
					xmlBuff.append("<rate><![CDATA[" + rate + "]]></rate>");
					xmlBuff.append("<rate__o><![CDATA[" + rate
							+ "]]></rate__o>");

					// rate__stduom
					xmlBuff.append("<rate__stduom><![CDATA[" + rate_stduom
							+ "]]></rate__stduom>");
					xmlBuff.append("<discount><![CDATA[" + discount
							+ "]]></discount>");
					xmlBuff.append("<req_date><![CDATA["
							+ sdf.format(reqDate).toString() + "]]></req_date>");
					xmlBuff.append("<req_date_o><![CDATA["
							+ sdf.format(reqDate).toString()
							+ "]]></req_date_o>");

					if (status.equals("O") || status.equalsIgnoreCase("O")) {
						status = "C";
						xmlBuff.append("<status><![CDATA[" + status
								+ "]]></status>");
					} else {
						xmlBuff.append("<status><![CDATA[" + status
								+ "]]></status>");
					...
					// xmlBuff.append("<status_date><![CDATA[" + statusDate +
					// "]]></status_date>");
					xmlBuff.append("<loc_code><![CDATA[" + locCode
							+ "]]></loc_code>");
					xmlBuff.append("<tax_class><![CDATA[" + taxClass
							+ "]]></tax_class>");
					xmlBuff.append("<tax_chap><![CDATA[" + taxChap
							+ "]]></tax_chap>");
					System.out.println("taxEnv@@before-->" + taxEnv);
					// taxEnv=taxEnv==null:"0":checkNull(rs.getString("tax_env"));
					xmlBuff.append("<tax_env><![CDATA[" + taxEnv
							+ "]]></tax_env>");

					System.out.println("taxEnv@@after-->" + taxEnv);
					xmlBuff.append("<remarks><![CDATA[" + remarks
							+ "]]></remarks>");
					xmlBuff.append("<work_order><![CDATA[" + WorkOrder
							+ "]]></work_order>");
					xmlBuff.append("<tax_amt><![CDATA[" + taxAmt
							+ "]]></tax_amt>");
					xmlBuff.append("<tot_amt><![CDATA[" + totAmt
							+ "]]></tot_amt>");
					xmlBuff.append("<dlv_qty><![CDATA[" + dlvQty
							+ "]]></dlv_qty>");
					xmlBuff.append("<unit__rate><![CDATA[" + unitRate
							+ "]]></unit__rate>");
					xmlBuff.append("<conv__qty_stduom><![CDATA["
							+ conv_qty_stduom + "]]></conv__qty_stduom>");
					xmlBuff.append("<conv__rtuom_stduom><![CDATA["
							+ conv_rtuom_stduom + "]]></conv__rtuom_stduom>");
					xmlBuff.append("<unit__std><![CDATA[" + unitStd
							+ "]]></unit__std>");
					xmlBuff.append("<quantity__stduom><![CDATA["
							+ quantityStduom + "]]></quantity__stduom>");
					xmlBuff.append("<no_art><![CDATA[" + noArt + "]]></no_art>");
					xmlBuff.append("<acct_code__dr><![CDATA[" + acct_dr
							+ "]]></acct_code__dr>");
					xmlBuff.append("<cctr_code__dr><![CDATA[" + cctr_dr
							+ "]]></cctr_code__dr>");
					xmlBuff.append("<acct_code__cr><![CDATA[" + acct_cr
							+ "]]></acct_code__cr>");
					xmlBuff.append("<cctr_code__cr><![CDATA[" + cctr_cr
							+ "]]></cctr_code__cr>");
					xmlBuff.append("<discount_type><![CDATA[" + discountType
							+ "]]></discount_type>");
					xmlBuff.append("<rate__clg><![CDATA[" + rateClg
							+ "]]></rate__clg>");
					xmlBuff.append("<specific_instr><![CDATA[" + specificInstr
							+ "]]></specific_instr>");
					xmlBuff.append("<special_instr><![CDATA[" + specialInstr
							+ "]]></special_instr>");
					xmlBuff.append("<emp_code__qcaprv><![CDATA[" + empCodeQcApr
							+ "]]></emp_code__qcaprv>");
					xmlBuff.append("<emp_code__qcaprv_o><![CDATA["
							+ empCodeQcApr + "]]></emp_code__qcaprv_o>");

					xmlBuff.append("<proj_code><![CDATA[" + projCode
							+ "]]></proj_code>");
					// xmlBuff.append("<spl_instr><![CDATA[" + specificInstr +
					// "]]></spl_instr>");
					// xmlBuff.append("<exch_rate><![CDATA[" + exchRate +
					// "]]></exch_rate>");
					xmlBuff.append("<dept_code><![CDATA[" + deptCode
							+ "]]></dept_code>");

					xmlBuff.append("<benefit_type><![CDATA[" + benefitType
							+ "]]></benefit_type>");
					xmlBuff.append("<licence_no><![CDATA[" + licenceNo
							+ "]]></licence_no>");
					// xmlBuff.append("<spec_metadata><![CDATA[" + specMetadata
					// + "]]></spec_metadata>");
					// xmlBuff.append("<spec_dimension><![CDATA[" +
					// specDimension + "]]></spec_dimension>");
					// xmlBuff.append("<supp_item__ref><![CDATA[" + suppItemRef
					// + "]]></supp_item__ref>");
					// xmlBuff.append("<quantity__fc><![CDATA[" + qtyFc +
					// "]]></quantity__fc>");
					// xmlBuff.append("<prd_code__rfc><![CDATA[" + prdCodeRfc +
					// "]]></prd_code__rfc>");
					xmlBuff.append("<form_no><![CDATA[" + formNo
							+ "]]></form_no>");
					xmlBuff.append("<duty_paid><![CDATA[" + dutyPaid
							+ "]]></duty_paid>");
					xmlBuff.append("<dlv_date><![CDATA["
							+ sdf.format(dlvDate).toString() + "]]></dlv_date>");
					xmlBuff.append("<site_code__o><![CDATA[" + siteCode
							+ "]]></site_code__o>");
					xmlBuff.append("<quantity__o><![CDATA[" + quantity
							+ "]]></quantity__o>");
					// xmlBuff.append("<rate_o><![CDATA[" + rate +
					// "]]></rate__o>");
					xmlBuff.append("<tax_class__o><![CDATA[" + taxClass
							+ "]]></tax_class__o>");
					xmlBuff.append("<tax_chap__o><![CDATA[" + taxChap
							+ "]]></tax_chap__o>");
					xmlBuff.append("<tax_env__o><![CDATA[" + taxEnv
							+ "]]></tax_env__o>");
					xmlBuff.append("<dlv_date__o><![CDATA["
							+ sdf.format(dlvDate).toString()
							+ "]]></dlv_date__o>");
					xmlBuff.append("<specific_instr__o><![CDATA["
							+ specificInstr + "]]></specific_instr__o>");
					// xmlBuff.append("<supp_code_mnfr__o><![CDATA[" +
					// suppCodeMnfr + "]]></supp_code_mnfr__o>");

					xmlBuff.append("<special_instr__o><![CDATA[" + specialInstr
							+ "]]></special_instr__o>");
					xmlBuff.append("<benefit_type__o><![CDATA[" + benefitType
							+ "]]></benefit_type__o>");
					xmlBuff.append("<licence_no__o><![CDATA[" + licenceNo
							+ "]]></licence_no__o>");
					xmlBuff.append("<form_no__o><![CDATA[" + formNo
							+ "]]></form_no__o>");
					xmlBuff.append("<duty_paid__o><![CDATA[" + dutyPaid
							+ "]]></duty_paid__o>");
					xmlBuff.append("<acct_code__dr__o><![CDATA[" + acct_dr
							+ "]]></acct_code__dr__o>");
					xmlBuff.append("<cctr_code__dr__o><![CDATA[" + cctr_dr
							+ "]]></cctr_code__dr__o>");
					xmlBuff.append("<acct_code__cr__o><![CDATA[" + acct_cr
							+ "]]></acct_code__cr__o>");
					xmlBuff.append("<cctr_code__cr__o><![CDATA[" + cctr_cr
							+ "]]></cctr_code__cr__o>");
					xmlBuff.append("<proj_code__o><![CDATA[" + projCode
							+ "]]></proj_code__o>");
			...
			//Calling saveDate master statefull function.
			...
			if (retString.indexOf("Success") > -1) {
				...
				System.out.println("Tran ID :::::in conf:::::::"
						+ arrayForTranId1);
				System.out.println("Tran ID :::::in conf:::::::"
						+ arrayForTranId1[1]);
				...
				if(errString==null||errString.trim().length()==0 ||  errString.indexOf("Success")>-1 || errString.equalsIgnoreCase("PRCUSUCCES")||errString.equals("PRCUSUCCES")||errString.equals("PRCUSUCCES"))
				{
					...
					try {
						double ordQty=0 ,totalOrdqty=0,indOrdQty=0;;
						Timestamp sysDate1 = null;
						SimpleDateFormat sdf1 = null;
						String sysDateStr1 = "",loginEmpCode="",errString1="";
						double indQty = 0,orderQty=0;
						int cnt = 0, detCnt = 0,mulPoIndRef=0;
						 int updCnt1 = 0,mulPoIndRefCnt=0,updCnt = 0;
							int indOQty=0;
...
//								xtraParams, "loginCode");
						...
						while (rs1.next()) {
							...
							if (indNo != null) {
								...
								sql = " select quantity,ord_qty from indent where  ind_no = ? ";
								pstmt = conn.prepareStatement(sql);
								...
								if (sumPoQty == indQty) {
								 if ((dlvQty > 0)) {
									 	System.out
										.println("comming into dlvQty>0 condition@@");
								       
										System.out.println("dlvQty@@At (sumPoQty == indQty)@(dlvQty>0)"+dlvQty);
										
										totalOrdqty = sumPoQty - dlvQty;
										System.out.println("totalOrdqty@@"+totalOrdqty);
										...
										if(sumPoQty==dlvQty){
										    sql=null;
											sql = " update indent set status = 'C', status_date=? ,ord_qty="+indOQty+" where  ind_no = ? ";
										}
										else if((sumPoQty == indQty)&&(sumPoQty>dlvQty))
										{
											//for partially Receipt
											sql=null;
											sql=" update indent set status = 'C', status_date=? ,ord_qty="+indOrdQty+" where  ind_no = ? ";
										...
										System.out
												.println("Indent status updated@@"
														+ updCnt1);
										pstmt.close();
										pstmt = null;
										sql = " update porddet set status = 'C' where  ind_no = ? and purc_order=? and line_no=? ";
										pstmt = conn.prepareStatement(sql);
										...
										System.out
												.println("porder status updated@@"+ updCnt);
										pstmt.close();
										pstmt = null;
									...
									System.out
											.println("comming into quantity==indQty condition@@");
									//updCnt = 0;
						...
						//			if(indOrdQty==0)
						//			{
									sql = " update indent set status = 'A' ,status_date=? ,ord_qty=?  where  ind_no = ? ";
						//			}else
						//			{
						//				sql = " update indent set status = 'A' ,status_date=? ,ord_qty=?  where  ind_no = ? ";
						//			}
									pstmt = conn.prepareStatement(sql);
									pstmt.setTimestamp(1, sysDate1);
									pstmt.setDouble(2, indOrdQty);
									pstmt.setString(3, indNo);
								...
								if (sumPoQty < indQty) {
									...
									if ((dlvQty > 0)) {
										...
										.println("comming into dlvQty>0 condition@@");
								         updCnt1 = 0;
								        System.out.println("dlvQty@@At (sumPoQty <indQty)@(dlvQty>0)"+dlvQty);
								        System.out.println("dlvQty@Partly@dlvQty>0 @"+dlvQty);
								        System.out.println("sumPoQty@Partly@dlvQty>0 @"+sumPoQty);
								        totalOrdqty = sumPoQty - dlvQty;
										System.out.println("totalOrdqty@@"+totalOrdqty);
										convIndOrdQty=distCommon.convQtyFactor(unit, unitStd, itemCode, totalOrdqty, conn)	;
										System.out.println("convIndOrdQty"+convIndOrdQty);
										...
										System.out.println("indOrdQty@Partly@[orderQty-convIndOrdQty]"+indOrdQty);
										sql=null;
										sql = " select count(1) from porddet where  ind_no = ? and status='O' ";
										pstmt = conn.prepareStatement(sql);

										pstmt.setString(1, indNo);
										rs = pstmt.executeQuery();
										if (rs.next()) {
											mulPoIndRef=rs.getInt(1);
											System.out.println("count@@ind in multiple po@cnt@"+mulPoIndRef);
										...
										if(sumPoQty==dlvQty){
											System.out
											.println("comming into Full RCP condition@(sumPoQty==dlvQty)@");
									
										    sql=null;
											sql = " update indent set status = 'C', status_date=?,ord_qty="+indOrdQty+"  where  ind_no = ? ";
										}
										 else if((mulPoIndRef>0) && (sumPoQty!=dlvQty) )
										{
											System.out
											.println("comming into multiple po ref against indent---RCP condition@(mulPoIndRef>1) && (sumPoQty!=dlvQty)@");
									
											sql=null;
											sql=" update indent set status = 'O', status_date=? ,ord_qty="+indOrdQty+" where  ind_no = ? ";
											
											
										}else if((cnt>0))
										{
											System.out
											.println("comming into multiple po ref against indent---RCP condition@(cnt>0)@");
										...
										{
											if(orderQty==0 || indOrdQty==0)
											{
												double orQty=0;
												System.out
												.println("comming into multiple po ref against indent---RCP condition@(orderQty==0)@");
										
												sql=null;
												sql=" update indent set status = 'A', status_date=? ,ord_qty="+orQty+" where  ind_no = ? ";
										...
										{
											System.out
											.println("comming into Partly RCP condition@(sumPoQty!=dlvQty)@");
									...
									System.out.println("convIndOrdQty"+convIndOrdQty);
									indOrdQty=orderQty-convIndOrdQty;
									System.out.println("ordQty1@@-->"+indOrdQty);
									System.out
										.println("ordQty1 calc@@"+indOrdQty);
									sql = " update porddet set status = 'C' where  ind_no = ? and purc_order=? and line_no=? ";
									pstmt = conn.prepareStatement(sql);
									pstmt.setString(1, indNo);
									pstmt.setString(2, purcOrder);
									...
									rs = pstmt.executeQuery();
									if (rs.next()) {
										mulPoIndRefCnt=rs.getInt(1);
										System.out.println("count@@ind in multiple po@cnt@"+mulPoIndRefCnt);
									...
									sql=null;
									if(mulPoIndRefCnt>0){
										System.out.println("mulPoIndRefCnt>0@dlvQty<=0");
										sql=null;
										sql = " update indent set status = 'O' , status_date=? ,ord_qty=? where  ind_no = ? ";
										
									}else if(cnt>0){
										sql=null;
										System.out.println("(cnt>0)@dlvQty<=0");
										sql = " update indent set status = 'O' , status_date=? ,ord_qty=? where  ind_no = ? ";
										
									}else if(mulPoIndRefCnt<=0){
										if(indOrdQty==0 || indOrdQty==0.0)
										{
											System.out.println("mulPoIndRefCnt<=0@dlvQty<=0");
											sql=null;
											sql = " update indent set status = 'A' , status_date=? ,ord_qty=? where  ind_no = ? ";
										...
										System.out.println("mulPoIndRefCnt<=0@dlvQty<=0");
										sql=null;
										sql = " update indent set status = 'C' , status_date=? ,ord_qty=? where  ind_no = ? ";
										
										}
										
									}else  if(cnt==0)
									{
										System.out.println("comming into(cnt<=0)@dlvQty<=0");
										if(indOrdQty==0){
											System.out.println("(cnt<=0)@ordQty==0@dlvQty<=0");
										sql = " update indent set status = 'A' , status_date=? ,ord_qty=? where  ind_no = ? ";
										}else
										{
											System.out.println("(cnt<=0)@dlvQty<=0");
									...
									}else if(indOrdQty==0 || indOrdQty==0.0)
									{
										sql=null;
										System.out.println("mulPoIndRefCnt<=0@dlvQty<=0@ordQty1==0");
									sql = " update indent set status = 'A' , status_date=? ,ord_qty=? where  ind_no = ? ";
									}else  
									{
										sql=null;
										System.out.println("mulPoIndRefCnt<=0@dlvQty<=0@ordQty1>0");
										//one indent used in different po(Partially). 
									sql = " update indent set status = 'O' , status_date=? ,ord_qty=? where  ind_no = ? ";
						...
						try {
							...
							errString1 = itmDBAccessEJB.getErrorString(" ", "VMPOSUC", userId,"",conn);
							return errString1;
						...
						{
							if(errString1!=null || errString1.trim().length()>0||errString1.indexOf("VMPOSUC")>-1)
							{
				...
				System.out.println("After confiremed error String-->"
						+ errString);
			...
			try {
				...
				System.out.println("Exception : PorderClosePrcEJB =>"
						+ d.toString());
				d.printStackTrace();
				System.out
						.println("Exception :PorderClosePrcEJB :process(String xmlString2, String xmlString2, String windowName, String xtraParams):"
								+ e.getMessage() + ":");
				e.printStackTrace();
				errString = e.getMessage();
				throw new ITMException(e);
	...
	public String saveData(String siteCode, String xmlString, String userId, Connection conn)
			throws ITMException {
		...
		try {
			...
			System.out.println("-----------masterStateful------- "
					+ masterStateful);
		...
		//	UserInfoBean userInfo = getUserInfo();
			...
			System.out.println("ITMException :PorderClosePrc :saveData :==>");
			throw itme;
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("Exception :PorderClosePrc :saveData :==>");
			throw new ITMException(e);
		}
		return retString;
	}
