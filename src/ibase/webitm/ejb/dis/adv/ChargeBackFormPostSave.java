...
public class ChargeBackFormPostSave extends ValidatorEJB implements
ChargeBackFormPostSaveLocal, ChargeBackFormPostSaveRemote
	...
	public String postSave(String winName,String editFlag,String tranId,  String xtraParams,Connection conn) throws RemoteException,ITMException
	{
		...
		try {
			...
			pstmt.close();
			pstmt = null;
			sql = "UPDATE charge_back_form set amount =  ? "
					+ " WHERE  tran_id = ? ";
			pstmt = conn.prepareStatement(sql);
			...
			pstmt.close();
			pstmt = null;
			sql = "UPDATE charge_back_form set net_amt =  ?,tax_amt = ?"
					+ " WHERE  tran_id = ? ";
			pstmt = conn.prepareStatement(sql);
