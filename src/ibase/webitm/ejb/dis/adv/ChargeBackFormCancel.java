...
public class ChargeBackFormCancel extends ActionHandlerEJB implements ChargeBackFormCancelLocal,ChargeBackFormCancelRemote
{
	int updateCount=0;
    ...
    public String cancel(String tranID, String xtraParams, String forcedFlag) throws RemoteException, ITMException
    {
		...
		System.out.println("Inside ChargeBackFormCancel cancel() method");
		...
		try
        {
	        ...
	        sql = " select  count(*) as cnt from charge_back_form where tran_id = ? and confirmed = 'Y'";
			...
			if(cnt > 0)
			{
				errString = itmDBAccessEJB.getErrorString("","VTALRACCNC","","",conn);
			    return errString;
			}
//			else
//			{
				 ...
				 * */
			System.out.println("Tran Id fornd by transaction :- ["+tranID+"]");
	        sql = " select  count(*) as cnt from charge_back_form where tran_id = ? and STATUS = 'X'";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, tranID.trim());
			rs = pstmt.executeQuery();
			if(rs.next()) 
			...
			else
			{
				...
				sql = "UPDATE charge_back_form set status='X',STATUS_DATE=? where tran_id=?";
				pstmt = conn.prepareStatement(sql);
