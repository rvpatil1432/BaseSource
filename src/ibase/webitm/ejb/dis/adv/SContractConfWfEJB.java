...
public class SContractConfWfEJB extends ActionHandlerEJB implements SContractConfWfEJBLocal, SContractConfWfEJBRemote 
{
	public String confirm(String tranId, String xtraParams, String forcedFlag) throws RemoteException, ITMException 
	{
		...
		try
		{
			errString = confirm(tranId, xtraParams, forcedFlag, userInfoStr);//changed from userInfoStr to errString by Jaffar S. on 11-01-19 for getting message on front end
				...
				else if ("O".equalsIgnoreCase(status) || "".equalsIgnoreCase(status)) 
				{
					if("O".equalsIgnoreCase(wf_status) || "R".equalsIgnoreCase(wf_status) || "".equalsIgnoreCase(wf_status))
					{
						...
						if ("success".equalsIgnoreCase(retString)) {
							...
							sql = "UPDATE SCONTRACT SET wf_status = 'S' WHERE CONTRACT_NO = ? ";
							pStmt = conn.prepareStatement(sql);
							pStmt.setString(1, tranId);
