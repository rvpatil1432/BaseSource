...
@Stateless 
public class EmpTravelInfoTicket extends ActionHandlerEJB implements EmpTravelInfoTicketRemote, EmpTravelInfoTicketLocal
{
	...
	public String confirm( String tranId, String xtraParams, String forcedFlag, Connection conn, boolean connStatus ) throws RemoteException,ITMException
	{
		...
		try
		{
			...
			{	sql = " select count(*) from emp_travel_info where tran_id = ?  and (case when ticket_status is null then 'N' else ticket_status end = 'Y') ";
				...
				if(count1 > 0 )
				{
					...
					retString = itmDBAccess.getErrorString("","VTTCKTCONF","","",conn);
					return retString;					
				}
				else
				{					
		...
		finally
		{
			if(retString != null && retString.indexOf("submited") > -1)
			{
				...
				retString = itmDBAccess.getErrorString("","VTTKTSUC","","",conn);
			}
			else
			{	
				retString = itmDBAccess.getErrorString("","VTERRTIKT","","",conn);
			}
			try{
